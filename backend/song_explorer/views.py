...
class SavedSongView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            try:
                serializer.save()
                return Response({"message": "Song saved successfully"}, status=status.HTTP_201_CREATED)
...
class SavedSongDetailView(APIView):
    def patch(self, request, song_id, *args, **kwargs):
        ...
        if track_rating is not None:
            ...
            song.save()
            return Response(
                {"message": "Track rating updated successfully", "track_rating": song.track_rating},
                status=status.HTTP_200_OK
                )
        return Response(
            {"message": "Invalid track rating."},
            status=status.HTTP_400_BAD_REQUEST
        )
